...
from freedpp.env.reward import Reward, MolLogP, PatternFilter, OutOfRange, identity, BRSASCORE
...
from freedpp.args_new import parse_args, update_args
...
def get_difference(args):
    DV1 = DockingVina(args['docking_config_1'])['DockingScore_1Score']
    DV2 = DockingVina(args['docking_config_2'])['DockingScore_2Score']
    difference = float(DV1)-float(DV2)
    return difference
'''
...
def init_rewards(args):
    ...
    costs = {
        ...
        'DockingScore_2': Reward(DockingVina(args['docking_config_2']), partial(max, 0)),
        #'Difference_docking': Reward(get_difference, partial(min, 0)),
        ...
        'BRSASCORE': Reward(BRSASCORE, OutOfRange(lower=1, upper=5, hard=hard), preprocess=preprocess)
    }
    rewards = dict()
