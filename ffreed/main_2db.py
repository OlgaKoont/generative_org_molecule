...
from ffreed.args_2db import parse_args, update_args
...
def init_rewards(args):
    ...
    costs = {
        'DockingScore_1': Reward(DockingVina(args['docking_config_1']), identity),
        'DockingScore_2': Reward(DockingVina(args['docking_config_2']), identity),
        ...
        'DBAffinity_1': Reward(DBAffinity(args['db_config_1']), identity, batched_mode=True),
        'DBAffinity_2': Reward(DBAffinity(args['db_config_2']), identity, batched_mode=True),
    }

    rewards = dict()
    for name, weight in zip(args['objectives'], args['weights']):
        costs[name].weight = -weight
        processes = args['num_sub_proc'] if name in ['DockingScore_1', 'DockingScore_2'] else 1
        if name in ['DockingScore_1', 'DockingScore_2']:
            rewards[name] = CacheAndPool(costs[name], processes=processes)
        elif name in ['DBAffinity_1', 'DBAffinity_2']:
