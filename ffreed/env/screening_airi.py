...
from datasets.utils.diffusion_utils import t_to_sigma as t_to_sigma_compl, get_t_schedule, set_time
from datasets.utils.sampling import randomize_position, sampling
from datasets.utils.utils import get_model
from datasets.utils.visualise import LigandToPDB, modify_pdb, receptor_to_pdb, save_protein
from datasets.utils.clash import compute_side_chain_metrics
...
parser.add_argument('--batch_size_db', type=int, default=32, help='')
...
I = int(np.ceil(len(data_list) / args.batch_size_db))
for i in range(I):
    batch = data_list[i*args.batch_size_db:(i+1)*args.batch_size_db]
