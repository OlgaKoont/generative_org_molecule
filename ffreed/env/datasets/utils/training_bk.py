...
def loss_function(lddt_pred, affinity_pred, tr_pred, rot_pred, tor_pred, res_tr_pred, res_rot_pred, res_chi_pred, data, t_to_sigma, device, lddt_weight=1, affinity_weight=1, tr_weight=1, rot_weight=1,
    ...
    affinity_loss = ((affinity_pred.cpu() - affinity) ** 2).mean(dim=mean_dims)
    ...
    affinity_base_loss = (affinity ** 2).mean(dim=mean_dims).detach()
    ...
    res_chi_loss = ((1-(res_chi_pred.cpu()-res_chi_score).cos())*res_loss_weight*res_chi_mask).sum(dim=mean_dims) / (res_chi_mask.sum(dim=mean_dims)+1e-12) * 3.0
...
def finetune_epoch(model, complex_graphs, device, t_to_sigma, args, optimizer, loss_fn, ema_weights):
    model.eval()
    t_schedule = get_t_schedule(inference_steps=args.inference_steps)
    tr_schedule, rot_schedule, tor_schedule, res_tr_schedule, res_rot_schedule, res_chi_schedule = t_schedule, t_schedule, t_schedule, t_schedule, t_schedule, t_schedule
        ...
        if (si+1) % args.sample_batch_size != 0 and si < len(complex_graphs):
            ...
            for i,data in enumerate(predictions_list):
                ...
                score = 0.25 * ((ca_lig_cross_distances_diff<0.5).float()
                                ...
                                + (ca_lig_cross_distances_diff<4.0).float())
                ca_lig_cross_lddt =  (score * cutoff_mask).sum() / (cutoff_mask.sum()+1e-12)
                all_lddt.append(ca_lig_cross_lddt)
                data.lddt = ca_lig_cross_lddt[None,None]

                center = orig_complex_graph['ligand'].pos.numpy().mean(0,keepdims=True)
                tran, rot = get_align_rotran(orig_complex_graph['ligand'].pos.numpy()-center,data['ligand'].pos.numpy()-center)
                if np.linalg.norm(tran) > 4*args.tr_sigma_max: continue
                tr_update = torch.from_numpy(tran).float()
                rot_update = Rotation.from_matrix(rot.T).as_rotvec()


                tr_t = max(min(np.log(np.linalg.norm(tran)/2/0.8/args.tr_sigma_min) / np.log(args.tr_sigma_max/args.tr_sigma_min),1.),0.)
                rot_t = max(min(np.log(np.linalg.norm(rot_update)/2/args.rot_sigma_min) / np.log(args.rot_sigma_max/args.rot_sigma_min),0.69),0.)
                t = max(tr_t,rot_t)
                lddt_data = copy.deepcopy(data)
                set_time(lddt_data, 0., 0., 0., 0., 0., 0., 1, args.all_atoms, None)
                lddt_data_list.append(lddt_data)

                set_time(data, tr_t, rot_t, t, t, t, t, 1, args.all_atoms, None)

                data.tr_score = -tr_update
                data.rot_score = -torch.from_numpy(rot_update).float().unsqueeze(0)
                data.tor_score = None if args.no_torsion else -(data.torsion_updates % (2*np.pi)).float()

                data.res_tr_score = -data.res_tr_update
                res_rotvec_update = Rotation.from_matrix(data.res_rot_update.cpu().numpy()).as_rotvec()
                data.res_rot_score = -torch.from_numpy(res_rotvec_update).float()#torch.from_numpy(so3.score_res_vec(vec=res_rot_update, eps=res_rot_sigma.cpu().numpy())).float()
                data.res_chi_score = -((data.res_chi_update * data['receptor'].chi_masks) % (2*np.pi))

                # print(data.tr_score)
                # print(data.rot_score)
                # print(data.tor_score)
                # print(data.res_tr_score)
                # print(data.res_rot_score)
                # print(data.res_chi_score)
                # data.tor_sigma_edge = data.tor_sigma_edge[0]
                data.tor_sigma_edge = np.ones(data['ligand'].edge_mask.sum()) * (args.tor_sigma_min ** (1-t) * args.tor_sigma_max ** t)
                data.tor_loss_weight = torch.from_numpy(np.ones(data['ligand'].edge_mask.sum()) * (tr_t<0.7)).float()
    ...
    all_lddt = torch.stack(all_lddt).view(-1).cpu().numpy()
    pos_frac = (all_lddt>0.9).mean()
    if pos_frac > 0:
        affinity_pos_weight = 1. / pos_frac
    else:
        affinity_pos_weight = 1.

    loader_class = DataListLoader if torch.cuda.is_available() else DataLoader

    train_loader = loader_class(sample_data_list, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=True, pin_memory=args.pin_memory)
    finetune_losses = train_epoch(model, train_loader, optimizer, device, t_to_sigma, loss_fn, ema_weights, train_score=True, finetune=True)

    train_loader = loader_class(lddt_data_list, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=True, pin_memory=args.pin_memory)
    finetune_lddt_losses = train_epoch(model, train_loader, optimizer, device, t_to_sigma, loss_fn, ema_weights, train_score=False, finetune=True)

    return finetune_losses,finetune_lddt_losses
